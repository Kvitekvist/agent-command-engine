...
- **Screenshot Capture** — drag-to-select a region of the screen (including ACE itself) from any running agent's card; saves into that project's own `.ace/screenshots/` folder (auto-`.gitignore`d) with the path copied to the clipboard, ready to paste into the agent's terminal
