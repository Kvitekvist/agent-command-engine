...
- **PathService** (TICKET-0060): at startup (`applyLoginShellPath()`, called at the top of `app.whenReady()` before the ptyHost fork / any spawn), resolves the user's real interactive-login-shell PATH via `$SHELL -ilc 'printf …$PATH…'` and merges it into `process.env.PATH`. Fixes the macOS/Linux GUI-launch trap where a Finder/Dock-launched app inherits only `/usr/bin:/bin:/usr/sbin:/sbin` and can't find `claude`/`codex` (on `~/.local/bin`, Homebrew, nvm) — so `prereqs:check` and the agent PTY finally agree on one PATH. No-op on Windows; fails open (keeps inherited PATH) on any error. `parseShellPath`/`mergePaths` are pure + unit-tested
...
src/
  ...
  main/           — Electron main process
    services/     — AgentService, DBService, LoadBalancer, TokenParser, TerminalService, FileService, ScreenshotService (TICKET-0034), PathService (TICKET-0060)
