...
### Added
- Regression test coverage for the PTY / agent-terminal spawn fix (`tests/pty-perms.test.js`): verifies `fix-pty-perms` discovers every node-pty `spawn-helper` (prebuilds + build/Release, ignoring decoys) and restores the executable bit that `posix_spawnp failed` on, and that ptyHost's new `describeSpawnError` appends the fix hint only on a POSIX `posix_spawnp` failure (never on Windows). Runs dependency-free against a temp fake node-pty tree. Suite is 44 tests. Refactored `fix-pty-perms.js` and `ptyHost.js` to export the now-tested helpers (TICKET-0068)
